        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-cente">
                ...
                <div class="form-group mr-1 text-end">
                    <a class="btn btn-primary" href="{{ route('pajak.create') }}">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @else
                                    <span class="badge bg-warning">{{ rp($row->total) }}</span>
                                @endif
                            <td>
                                @if($row->status == 1)
                            ...
                            <td>
                                <a class="btn btn-sm btn-secondary" href="{{ route('pajak.show', $row) }}">
                                    <i class="bi bi-eye-fill"></i>
                                </a>
                                @if($row->status == 1)
                                <a class="btn btn-sm btn-info disabled" tabindex="-1" role="button" aria-disabled="true" href="{{ route('pajak.edit', $row) }}">
                                    <i class="bi bi-pencil-square"></i>
                                </a>
                                <form action="{{ route('pajak.destroy', $row) }}" method="POST" style="display: inline-block;"
                                ...
                                <a class="btn btn-sm btn-info" href="{{ route('pajak.edit', $row) }}">
                                    <i class="bi bi-pencil-square"></i>
                                </a>
                                <form action="{{ route('pajak.destroy', $row) }}" method="POST" style="display: inline-block;"
                                    onsubmit="return confirm('Hapus Data?')">
